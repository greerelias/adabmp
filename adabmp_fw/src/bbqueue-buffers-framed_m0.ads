...
package BBqueue.Buffers.Framed_M0
  with Preelaborate, SPARK_Mode, Abstract_State => null
is

   Max_Frame_Header_Size : constant := 9;

   subtype Framed_Count is
     Count range Count'First .. Count'Last - Max_Frame_Header_Size;
   ...
   type Framed_Buffer (Size : Buffer_Size) is limited private;
   ...
   with
     Pre  => State (G) /= Valid,
     Post =>
       State (G) in Valid | Empty | Grant_In_Progress | Insufficient_Size
       and then (if State (G) = Valid then Write_Grant_In_Progress (This));
   ...
   with
     Pre  => State (G) = Valid,
     Post =>
       (if Write_Grant_In_Progress (This)'Old
        then State (G) = Empty
        else State (G) = Valid);
   ...
   generic
      with
        procedure Process_Write
          (Data : out Storage_Array; To_Commit : out Count);
   procedure Write_CB
     (This   : in out Framed_Buffer;
      Size   : Framed_Count;
   ...
   procedure Read (This : in out Framed_Buffer; G : in out Read_Grant)
   with
     Pre  => State (G) /= Valid,
     Post =>
       State (G) in Valid | Empty | Grant_In_Progress
       and then (if State (G) = Valid then Read_Grant_In_Progress (This));
   ...
   procedure Release (This : in out Framed_Buffer; G : in out Read_Grant)
   with
     Pre  => State (G) = Valid,
     Post =>
       (if Read_Grant_In_Progress (This)'Old
        then State (G) = Empty
        else State (G) = Valid);
   ...
   procedure Read_CB (This : in out Framed_Buffer; Result : out Result_Kind);
