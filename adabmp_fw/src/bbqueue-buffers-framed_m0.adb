-- Modified to Cortex M0

package body BBqueue.Buffers.Framed_M0
  with SPARK_Mode
is
   ...
   pragma
     Compile_Time_Error
       ((Count'Object_Size mod System.Storage_Unit) /= 0,
        "Invalid Object_Size for Count");

   procedure Write_Header
     (After_Hdr_Addr : System.Address; Hdr_Size : Header_Count; Value : Count);

   procedure Read_Header
     (Before_Hdr_Addr : System.Address;
   ...
   is
      ...
      -- Must treat header as array for write that aren't word aligned
      -- otherwise write will fail on Cortex M0
      Header : Storage_Array (1 .. Storage_Offset (Hdr_Size))
      with
        Address =>
          To_Address
            (To_Integer (After_Hdr_Addr) - Integer_Address (Hdr_Size));

      Val_Bytes : Storage_Array (1 .. Storage_Offset (Hdr_Size))
   ...
   is
      ...
      -- Byte-views for copying
      Src_Bytes : Storage_Array (1 .. Count'Object_Size / 8)
      with Address => Before_Hdr_Addr;
      Dst_Bytes : Storage_Array (1 .. Count'Object_Size / 8)
      with Address => Temp_Val'Address;
   begin
      -- Copy bytes from potentially unaligned buffer to aligned temp variable
      Dst_Bytes := Src_Bytes;
   ...
   procedure Grant
     (This : in out Framed_Buffer; G : in out Write_Grant; Size : Framed_Count)
   is
   ...
   procedure Commit
     (This : in out Framed_Buffer;
      ...
      Size : Framed_Count := Framed_Count'Last) is
   begin
      if Size = 0 then
         --  Nothing to commit
   ...
   begin
      BBqueue.Buffers.Release
        (This.Buffer, G.Grant, G.Header_Size + This.Current_Read_Size);
      G.Header_Size := 0;
   end Release;
   ...
   procedure Read_CB (This : in out Framed_Buffer; Result : out Result_Kind) is
      ...
      procedure Call_CB (Addr : System.Address; Length : Framed_Count);

      procedure Call_CB (Addr : System.Address; Length : Framed_Count) is
         pragma SPARK_Mode (Off);
