...
package AdaBMP_FW is
   ...
   B64E         : constant UInt32 := 16#D8#; -- Block 64 Erase
   B32E         : constant UInt32 := 16#52#; -- Block 32 Erase
   SE           : constant UInt32 := 16#20#; -- Sector Erase
   Block64_Size : constant UInt32 := 2 ** 16;
   Block32_Size : constant UInt32 := 2 ** 15;
   Sector_Size  : constant UInt32 := 2 ** 12;
   Page_Size    : constant UInt32 := 2 ** 8;
   -- PP right padded full word because we are using
...
private
   ...
   -- Erases from 64kb aligned base address
   procedure Run_Erase_Flash (Erase_Data : HAL.UInt8_Array);
